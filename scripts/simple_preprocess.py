...
def preprocess_vcf(
    ...
    try:
        ...
        logger.info(
            f"Successfully opened input VCF with {sum(1 for _ in vcf_in)} variants"
        )
    ...
    # Statistics
    stats = {"total": 0, "decomposed": 0, "left_shifted": 0, "skipped": 0}
            ...
            if left_shift and len(record.ref) != len(record.alts[0]):
                ...
                try:
                    ...
                    if len(record.ref) > len(record.alts[0]):  # Deletion
                        base_to_check = reference.fetch(
                            record.contig, record.pos - 2, record.pos - 1
                        )
...
def main():
    ...
    parser.add_argument(
        "--pass-only", action="store_true", help="Only include PASS variants"
    )
    parser.add_argument(
