#!/usr/bin/env python3
"""
Standalone demo for the Python preprocess implementation.

This script demonstrates the functionality of the PreprocessEngine
without relying on the rest of the hap.py infrastructure.
"""
...
class PreprocessEngine:
    ...
    This class implements variant preprocessing with pure Python using pysam.
    ...
    def _open_files(self):
        ...
        try:
            self.vcf_in = pysam.VariantFile(self.input_vcf)
            logger.info(f"Opened input VCF: {self.input_vcf}")
        except Exception as e:
            raise ValueError(f"Failed to open input VCF: {e}")
            
        try:
            self.reference = pysam.FastaFile(self.reference_fasta)
        ...
        if self.regions:
            if os.path.exists(self.regions):
                try:
                    self.region_tabix = pysam.TabixFile(self.regions)
    ...
    def _prepare_output_header(self):
        ...
        self.vcf_out = pysam.VariantFile(self.output_vcf, mode=mode, header=header)
        
    def _parse_bed_file(self, bed_file: str):
    ...
    def is_in_regions(self, record):
        """Check if a variant record is in the specified regions."""
        if not self.regions:
            return True
            
        if hasattr(self, 'region_tabix'):
            ...
            for region in self.region_list:
                if ":" in region and "-" in region:
                    ...
                    if (
                        record.contig == r_chrom and
                        record.pos >= r_start and
                        record.pos <= r_end
                    ):
                        return True
    ...
    def left_shift_variant(self, chrom, pos, ref, alt):
        """
    ...
    def decompose_variant(self, record):
        """
        ...
        if (
            self.decompose_level == DecomposeLevel.NONE or
            len(record.alts) <= 1
        ):
            return [record]
        ...
        for i, alt in enumerate(record.alts):
            ...
            for key, value in record.info.items():
                if key in new_record.header.info:
                    try:
                        ...
                        logger.warning(f"Failed to copy INFO field {key}: {e}")
            ...
            try:
                ...
                logger.warning(f"Failed to set ORIGINAL_ALLELES: {e}")
            ...
            if self.left_shift or self.decompose_level == DecomposeLevel.AGGRESSIVE:
                ...
                if (new_pos != new_record.pos or
                    new_ref != new_record.ref or
                    new_alt != alt):
    ...
    def handle_haploid_regions(self, record):
        """
    ...
    def process(self):
        ...
        """
        logger.info(f"Starting preprocessing with options: "
        ...
        logger.info(f"  Haploid adjusted variants: {self.stats['haploid_adjusted_variants']}")
...
def main():
    ...
    parser.add_argument(
        "input_vcf", help="Input VCF/BCF file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-o", "--output", 
        help="Output VCF/BCF file (default: auto-generate)"
    )
    parser.add_argument(
        "-V", "--decompose", type=int, default=1, choices=[0, 1, 2],
        help="Decomposition level: 0=none, 1=conservative, 2=aggressive (default: 1)"
    )
    parser.add_argument(
        "-L", "--left-shift", action="store_true", default=True,
        help="Left-align variants (default: True)"
    )
    parser.add_argument(
        "--no-left-shift", action="store_false", dest="left_shift",
    ...
    parser.add_argument(
        "--haploid-x", action="store_true",
        help="Treat X chromosome as haploid (convert het to hom)"
    )
    parser.add_argument(
        "--bcf", action="store_true",
        help="Output BCF instead of VCF"
    )
    parser.add_argument(
