...
class Python2Fixer:
    ...
    def _fix_file(self, file_path: Path, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            if content != fixed_content:
                ...
                if dry_run:
                    print(
                        f"🔍 Would fix {file_path.relative_to(self.project_root)}: {fix_list}"
                    )
                else:
                    with open(file_path, "w", encoding="utf-8") as f:
                        f.write(fixed_content)
                    print(
                        f"✅ Fixed {file_path.relative_to(self.project_root)}: {fix_list}"
                    )
    ...
    def _apply_fixes(self, content: str) -> Tuple[str, Dict[str, int]]:
        ...
        if "print " in content and not re.search(
            r"from __future__ import print_function", content
        ):
        ...
        if re.search(r"except\s+\w+\s*,\s*\w+\s*:", content):
            content = re.sub(
                r"except\s+(\w+)\s*,\s*(\w+)\s*:", r"except \1 as \2:", content
            )
            fixes["old_style_except"] += len(
                re.findall(r"except\s+\w+\s*,\s*\w+\s*:", content)
            )

        # Fix old-style super calls
        if "super(" in content:
            # This is a complex transformation, so only handle simple cases
            content = re.sub(r"super\((\w+),\s*self\)\.(\w+)", r"super().\2", content)
            fixes["old_style_super"] += len(
                re.findall(r"super\((\w+),\s*self\)\.(\w+)", content)
            )
        ...
        if "StringIO" in content:
            if "from StringIO import StringIO" in content:
                ...
                content = re.sub(r"StringIO\.StringIO\(", "io.StringIO(", content)
                fixes["stringio"] += 1
