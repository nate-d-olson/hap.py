...
class ModernizationReporter:
    ...
    def generate_report(self, detailed: bool = False) -> str:
        ...
        migrated_components = sum(1 for c, s in python_stats.items() if s["has_replacement"])
        progress_percentage = (migrated_components / total_components) * 100 if total_components > 0 else 0
...
"""
        ...
        for component, info in sorted(self.cpp_components.items()):
            ...
            py_info = python_stats.get(component, {"has_replacement": False, "files": 0, "loc": 0})
            ...
            if py_info["has_replacement"]:
                ...
                status = "❌ Pending"
                cpp_remaining = f"`{cpp_info['files']} C++ files, {cpp_info['loc']} lines`"
                py_details = cpp_remaining
            
            report += f"| {component} | {category} | {priority} | {status} | {py_details} |\n"
...
"""
        ...
        for pkg, info in dependency_stats.items():
            ...
            util = f"{info['utilization']}%" if info["utilization"] is not None else "N/A"
...
"""
        ...
        unmigrated = [c for c, s in python_stats.items() 
                     if not s["has_replacement"] and 
                     self.cpp_components[c]["priority"] == "high"]
        
        if unmigrated:
            report += f"1. **Priority Components to Migrate**: {', '.join(unmigrated)}\n"
        ...
        missing_deps = [p for p, i in dependency_stats.items() if not i["installed"] and p in ["pysam", "biopython", "pytest"]]
        if missing_deps:
            report += f"3. **Dependencies**: Add {', '.join(missing_deps)}\n"
        
        # Add detailed sections if requested
        if detailed:
            report += self._generate_detailed_sections(cpp_stats, python_stats, test_stats)
    ...
    def _analyze_cpp_components(self) -> Dict[str, Dict]:
        ...
        for component in self.cpp_components:
            cpp_files = list(self.cpp_dir.glob(f"**/*{component}*.cpp")) + list(self.cpp_dir.glob(f"**/*{component}*.h"))
    ...
    def _analyze_python_replacement_status(self) -> Dict[str, Dict]:
        ...
        for component in self.cpp_components:
            ...
            py_files = list(self.python_dir.glob(f"**/*{component}*.py")) + list(self.python_dir.glob(f"**/python_{component}*.py"))
                ...
                for py_file in self.python_dir.glob("**/*.py"):
                    try:
                        with open(py_file, 'r', encoding='utf-8') as f:
                            content = f.read()
                            if f"python_{component}" in content.lower() or f"{component}_python" in content.lower():
    ...
    def _analyze_test_coverage(self) -> Dict[str, Dict]:
        ...
        stats["unit"]["total"] = len([f for f in pytest_files if "integration" not in str(f)])
        stats["integration"]["total"] = len([f for f in pytest_files if "integration" in str(f)])
    ...
    def _analyze_code_quality(self) -> Dict[str, any]:
        ...
        metrics["Pre-commit Config"] = "✅ Present" if pre_commit_file.exists() else "❌ Missing"
        
        # Check pytest configuration
        pytest_conf = self.project_root / "pytest.ini"
        metrics["Pytest Config"] = "✅ Present" if pytest_conf.exists() else "❌ Missing"
                ...
                with open(py_file, 'r', encoding='utf-8') as f:
                    ...
                    for line in lines:
                        ...
                        if stripped.startswith('def ') and ':' in stripped:
                            total_functions += 1
                            if '->' in stripped or (': ' in stripped and 'def ' not in stripped.split(': ')[1]):
                                typed_functions += 1
            except Exception:
                pass
            ...
            try:
                with open(pyproject_file, 'r') as f:
                    ...
                    for package in dependencies:
                        dependencies[package]["installed"] = package.lower() in content.lower()
            except Exception:
                pass
...
"""
        ...
        for priority, components in priorities.items():
            migrated = sum(1 for c in components if python_stats.get(c, {}).get("has_replacement", False))
