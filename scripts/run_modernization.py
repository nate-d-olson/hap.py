...
def run_modernization(args):
    ...
    if args.pre_commit or args.all:
        ...
        success = run_command(precommit_cmd, "Running pre-commit hooks", check=False) and success
    ...
    if args.validate or args.all:
        validate_cmd = [
            sys.executable,
            str(scripts_dir / "validate_replacements.py")
        ]
        success = run_command(validate_cmd, "Validating Python replacements") and success
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run hap.py modernization steps"
    )
    parser.add_argument(
        "--all", action="store_true",
        help="Run all modernization steps"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fix-py2", action="store_true",
        help="Fix Python 2 artifacts"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Validate Python replacements"
    )
    parser.add_argument(
    ...
    if not (args.setup or args.fix_py2 or args.pre_commit or 
            args.report or args.validate or args.test or args.all):
        args.all = True
