...
class TestPhase3BasicFunctionality:
    ...
    def test_quantify_engine_phase3_parameters(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.vcf', delete=False) as truth_vcf, \
             tempfile.NamedTemporaryFile(mode='w', suffix='.vcf', delete=False) as query_vcf:
...
chr21\t27590000\t28100000\tlow_confidence_region"""
    ...
    def test_variant_region_assignment(self):
        ...
        try:
            ...
            test_variants = [
                {"chrom": "chr21", "pos": 27000000, "ref": "A", "alt": "G"},  # Within region
                {"chrom": "chr21", "pos": 30000000, "ref": "C", "alt": "T"},  # Outside region
            ]
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.bed', delete=False) as bed_file, \
             tempfile.NamedTemporaryFile(mode='w', suffix='.vcf', delete=False) as truth_vcf, \
             tempfile.NamedTemporaryFile(mode='w', suffix='.vcf', delete=False) as query_vcf:
