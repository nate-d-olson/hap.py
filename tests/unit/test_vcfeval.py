...
class TestVCFEval(unittest.TestCase):
    ...
    def test_findVCFEval(self):
        ...
        with patch("Haplo.vcfeval.has_vcfeval", True), \
    ...
    def test_runVCFEval_input_validation(self, mock_exists, mock_copy, mock_popen):
        ...
        # Mock for temp file
        vtf_mock = MagicMock()
        name_property = PropertyMock(return_value=os.path.join(self.temp_dir, "vcfeval.result_mock"))
        ...
        with patch('tempfile.NamedTemporaryFile', return_value=vtf_mock):
            ...
            with self.assertRaises(FileNotFoundError):
                vcfeval.runVCFEval(self.test_vcf1, self.test_vcf2, self.test_output, self.args)
    ...
    @patch("os.path.exists")
    @patch("os.path.isdir")
    def test_runVCFEval_default_params(self, mock_isdir, mock_exists, mock_copy, mock_popen):
        ...
        with patch('tempfile.NamedTemporaryFile', return_value=vtf_mock):
            ...
            result = vcfeval.runVCFEval(self.test_vcf1, self.test_vcf2, self.test_output, args)
    ...
    @patch("os.path.exists")
    @patch("os.path.isdir")
    def test_runVCFEval_subprocess_errors(self, mock_isdir, mock_exists, mock_copy, mock_popen):
    ...
    def test_runVCFEval_missing_output(self, mock_exists, mock_copy, mock_popen):
        ...
        # Setup mocks
        mock_exists.side_effect = lambda x: x != os.path.join(self.temp_dir, "output.vcf.gz")
        ...
        result = vcfeval.runVCFEval(self.test_vcf1, self.test_vcf2, self.test_output, self.args)
