...
applyTo: "**"
---
# Python 3 Migration Guidelines for Bioinformatics Code

## Migration Workflow

1. **Assessment Phase**
   - Run initial 2to3 analysis: `2to3 -p src/python/`
   - Create inventory of incompatible modules
   - Identify genomic data processing code requiring special attention
   - Determine priority components based on dependency graph

2. **Conversion Strategy**
   - Start with utility modules with few dependencies
   - Progress to core modules leveraging already-converted utilities
   - Leave entry points and scripts until supporting modules are converted
   - Handle Cython modules last with special care for memory management

3. **Testing Approach**
   - Create parallel Python 3 test environment
   - Run tests with both Python 2 and 3 during transition
   - Compare output files byte-by-byte for exact equivalence
   - Verify numeric results with appropriate tolerance for floating-point differences
