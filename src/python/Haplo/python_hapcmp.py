...
class HaploComparator:
    ...
    def _process_region(
        ...
        # Generate haplotypes
        try:
            block.haplotypes1 = self._generate_haplotypes(chrom, start, end, block.variants1)
            block.haplotypes2 = self._generate_haplotypes(chrom, start, end, block.variants2)
        except Exception as e:
            block.status = "error"
            ...
            for hap in haplotypes:
                ...
                for allele_idx in genotype:
                    if allele_idx is None or allele_idx == 0:
                        ...
                        new_hap = hap[:rel_pos] + alt + hap[rel_pos + len(variant.ref):]
    ...
    def _compare_haplotypes(self, haplotypes1: List[str], haplotypes2: List[str]) -> bool:
        """
    ...
    def _write_bed_record(self, bed_out, block: HaplotypeBlock) -> None:
        ...
        """
        bed_out.write(
            f"{block.chrom}\t{block.start}\t{block.end}\t{block.status}\n"
        )
...
def main():
    ...
    parser.add_argument(
        "-e", "--output-errors",
        help="Output failure information"
    )
    parser.add_argument(
        "-d", "--output-diffs",
    ...
    try:
        ...
        # Print summary
        logger.info(f"Processed {stats['blocks_processed']} of {stats['blocks_total']} blocks")
        ...
        if not args.output_bed:
            print(f"Processed {stats['blocks_processed']} of {stats['blocks_total']} blocks")
