...
class QuantifyEngine:
    ...
    def _load_variants(self, is_truth: bool = True):
        ...
        logger.info(f"Loaded {len(variants)} {vcf_name} variants "
    ...
    def _process_variant(self, variant):
        ...
        processed = {
            ...
            "is_indel": len(variant.ref) != len(variant.alts[0]) if variant.alts else False,
            "is_snp": len(variant.ref) == 1 and len(variant.alts[0]) == 1 if variant.alts else False,
            "is_mnp": len(variant.ref) > 1 and len(variant.ref) == len(variant.alts[0]) if variant.alts else False,
            "length": max(len(variant.ref), len(variant.alts[0])) if variant.alts else 0
        }
        ...
        if variant.samples:
            ...
            if "GT" in sample:
                ...
                processed["is_hom"] = sample["GT"][0] == sample["GT"][1] if len(sample["GT"]) > 1 else True
    ...
    def _in_regions(self, variant):
        ...
        for chrom, start, end in self.region_list:
            if (variant["chrom"] == chrom and 
                variant["pos"] >= start and 
                variant["pos"] <= end):
                return True
    ...
    def _match_variants(self):
        ...
        for (chrom, pos, ref, alt), group in grouped:
            if len(group) > 1 and 'truth' in group['source'].values and 'query' in group['source'].values:
    ...
    def _stratify_results(self):
        ...
        for size_range in [(1, 5), (6, 15), (16, 50), (51, float('inf'))]:
            range_name = f"{size_range[0]}-{size_range[1]}" if size_range[1] != float('inf') else f"{size_range[0]}+"
            
            # Filter indels by size
            truth_indels = [v for v in self.truth_variants 
                           if v['is_indel'] and size_range[0] <= v['length'] <= size_range[1]]
            query_indels = [v for v in self.query_variants 
                           if v['is_indel'] and size_range[0] <= v['length'] <= size_range[1]]
        ...
        for zygosity in ["HET", "HOM"]:
            ...
            truth_zyg = [v for v in self.truth_variants 
                        if v.get('is_hom', False) == is_hom]
            query_zyg = [v for v in self.query_variants 
                        if v.get('is_hom', False) == is_hom]
    ...
    def write_results(self, output_prefix: str):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"OVERALL\t{self.metrics['TP']}\t{self.metrics['FP']}\t{self.metrics['FN']}\t"
            ...
            for strat_type, strat_data in self.stratifications.items():
                for type_value, metrics in strat_data.items():
                    f.write(f"{strat_type.upper()}_{type_value}\t{metrics['TP']}\t{metrics['FP']}\t{metrics['FN']}\t"
...
def main():
    ...
    parser.add_argument("--apply-filters", action="store_true", help="Apply filters from VCF")
    parser.add_argument("--output-vtc", action="store_true", help="Output variant truth categories")
