...
class FastaReader:
    ...
    def get_sequence(self, chrom: str, start: int, end: int) -> str:
        ...
        if start < 0 or end > len(seq):
            raise IndexError(
                f"Invalid coordinates: {start}-{end} " f"(sequence length: {len(seq)})"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sequence manipulation utilities")
    parser.add_argument("--fasta", help="Path to a FASTA file")
    parser.add_argument("--complement", help="Sequence to complement")
    parser.add_argument("--reverse-complement", help="Sequence to reverse complement")
    parser.add_argument("--normalize", help="Sequence to normalize")
    parser.add_argument(
        "--get-sequence",
        nargs=3,
    ...
    if args.fasta:
        try:
            ...
            if args.get_sequence:
                ...
                sequence = fasta_reader.get_sequence(chrom, int(start), int(end))
