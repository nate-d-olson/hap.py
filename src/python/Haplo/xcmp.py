...
def xcmpWrapper(location_str, args):
    ...
    to_run = (
        "xcmp %s %s -l %s -o %s -r %s -f %i -n %i --expand-hapblocks %i "
    ...
    try:
        ...
        process = subprocess.Popen(
            to_run, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        ...
        if process.returncode != 0:
            raise subprocess.CalledProcessError(
                process.returncode, to_run, output=stdout, stderr=stderr
            )
