...
class VCFChecker:
    ...
    def _check_variant(self, record: VariantRecord) -> List[Tuple[str, str]]:
        ...
        if record.chrom in self.last_pos and record.pos <= self.last_pos[record.chrom]:
            issues.append(("OVERLAP", f"Overlaps with previous variant at position {self.last_pos[record.chrom]}"))
    ...
    def _check_reference_allele(self, record: VariantRecord) -> List[Tuple[str, str]]:
        ...
        try:
            ...
            ref_seq = self.reference.fetch(record.chrom, record.pos - 1, record.pos - 1 + len(record.ref))
            
            # Compare with the record's reference allele
            if ref_seq.upper() != record.ref.upper():
                issues.append(("REF_MISMATCH", f"Reference allele mismatch: VCF={record.ref}, Reference={ref_seq}"))
        ...
        for sample in record.samples:
            ...
            for allele_idx in gt:
                if allele_idx is not None and allele_idx > max_allele_idx:
                    issues.append(("INVALID_GT", f"Sample {sample} has invalid allele index {allele_idx}"))
                    self.stats["invalid_genotypes"] += 1
...
def main():
    ...
    parser.add_argument(
        "input_file", help="Input VCF/BCF file to check"
    )
    parser.add_argument(
        "-r", "--reference", help="Reference FASTA file for checking reference alleles"
    )
    parser.add_argument(
        "-o", "--output", help="Output file to write issues to"
    )
    parser.add_argument(
