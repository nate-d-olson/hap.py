...
def parseArgs():
    ...
    parser.add_argument("--no-write-counts", dest="writeCounts",
    ...
    parser.add_argument("--confident-regions-query", dest="conf_query",
    ...
    parser.add_argument("-X", "--roc", dest="roc",
    ...
    parser.add_argument("--engine-vcfeval-path", dest="vcfeval_path",
                      help="Path to RTG Tools vcfeval executable",
                      default=None)

    parser.add_argument("--engine-vcfeval-template", dest="vcfeval_template",
                      help="Path to RTG Tools SDF template",
                      default=None)

    parser.add_argument("--preserveAllVariants", dest="preserve_all_variants",
    ...
    parser.add_argument("--gender", dest="gender",
                       help="Gender, determines how to handle X/Y", default="female",
                       choices=["male", "female", "auto", "none"])

    parser.add_argument("--window-size", dest="window",
                      help="Preprocessing window size. Set this to 0 to disable windowing.",
                      default=20000, type=int)

    parser.add_argument("--xcmp-enumeration-threshold", dest="enum_threshold",
                      help=("Maximum number of enumerations per window for xcmp (use > 0 to disable "
                            "window-based decomposition of difficult regions)."),
                      default=10, type=int)

    parser.add_argument("--pass-only", dest="pass_only",
    ...
    parser.add_argument("--somatic-truth-id", dest="truthID",
                      help="For scmp-somatic truth VCF ID ",
                      default=None)

    parser.add_argument("--somatic-query-id", dest="queryID",
                      help="For scmp-somatic query VCF ID ",
                      default=None)

    parser.add_argument("--somatic-truth-bam", dest="truthbam",
                      help="For scmp-somatic truth BAM file ",
                      default=None)

    parser.add_argument("--somatic-query-bam", dest="querybam",
                      help="For scmp-somatic query BAM file ",
                      default=None)

    parser.add_argument("--set-globals", dest="globals", help="Set global flagss for preprocessing",
                      default=None)
    ...
    parser.add_argument("--preprocess-truth", dest="preprocess_truth",
                      help=("Preprocessing flag string for truth, e.g. DECOMPOSE,\
                            NO_COMPLEX_BRANCH,NO_LEFTSHIFT"),
                      default=None)

    parser.add_argument("--preprocess-query", dest="preprocess_query",
                      help=("Preprocessing flag string for query, e.g. DECOMPOSE,\
                            NO_COMPLEX_BRANCH,NO_LEFTSHIFT"),
                      default=None)

    parser.add_argument("--preprocess-window", dest="preprocess_window",
                      help="Window size for preprocessing",
                      type=int, default=1000)

    parser.add_argument("--write-vcf", dest="write_vcf",
    ...
    parser.add_argument("--no-fixchr-truth", dest="fixchr_truth",
                      help="Do not fix chr prefixes in truth",
                      action="store_false", default=True)

    parser.add_argument("--no-fixchr-query", dest="fixchr_query",
...
def main():
    ...
    try:
        ...
        if args.prefix is not None and os.path.exists(args.prefix + ".summary.csv") and not args.forceoverwrite:
            # check if the output file exists already
            logging.error("Results file %s exists already. Use --force-overwrite to overwrite." % (args.prefix + ".summary.csv"))
            return 1
        ...
        if args.engine == "xcmp":
            ...
            if args.roc:
                ...
                try:
                    Haplo.happyroc.makeRocs(args.prefix, filter_key, args.roc_delta, args.type)
