...
def fix_truncated_file(file_path):
    ...
    logging.info(f"Adapting backup file with Python 3 compatibility fixes: {backup_path}")
    try:
        # Read backup file
...
def fix_python3_compatibility(content):
    ...
    # Fix shebang line
    content = re.sub(r'#!/usr/bin/env python\s*$', 
                     '#!/usr/bin/env python3', 
                     content, 
                     flags=re.MULTILINE)
    
    # Fix 'except X, e:' to 'except X as e:'
    content = re.sub(r'except\s+([A-Za-z0-9_]+),\s+([A-Za-z0-9_]+)(\s*:)', 
                     r'except \1 as \2\3', 
                     content)
    
    # Fix file open operations for text files to include encoding
    content = re.sub(r'open\((.*?),\s*["\']r["\']\)', 
                     r'open(\1, "r", encoding="utf-8")', 
                     content)
    content = re.sub(r'open\((.*?),\s*["\']w["\']\)', 
...
def main():
    ...
    for file_path in TRUNCATED_FILES:
        ...
        if fix_truncated_file(file_path):
            ...
            print(f"  Fixed: {file_path} (now has {original_lines} lines, backup has {backup_lines} lines)")
            logging.info(f"  Fixed: {file_path} (now has {original_lines} lines, backup has {backup_lines} lines)")
        else:
            failed_count += 1
