...
def check_cmake_files(fix=False):
    ...
    for cmake_file in CMAKE_FILES:
        ...
        if (
            "find_package(PythonInterp" in content
            and "find_package(Python3" not in content
        ):
            issues_found += 1
            logger.info(
                f"{cmake_file}: Uses deprecated FindPythonInterp instead of FindPython3"
            )
...
def check_dependencies():
        ...
        try:
            ...
            if dep == "numpy":
                ...
                if (
                    ...
                    and int(numpy.__version__.split(".")[1]) < 16
                ):
                    outdated_deps.append(
                        f"{dep} (version {numpy.__version__}, need 1.16+)"
                    )
            elif dep == "cython":
                import Cython

                if (
                    hasattr(Cython, "__version__")
                    and Cython.__version__.startswith("0.")
                    and int(Cython.__version__.split(".")[1]) < 29
                ):
                    outdated_deps.append(
                        f"{dep} (version {Cython.__version__}, need 0.29+)"
                    )
    ...
    if outdated_deps:
        ...
        logger.info(
            "Update with: pip install --upgrade "
            + " ".join(dep.split()[0] for dep in outdated_deps)
        )
    ...
    message(WARNING "Cython not found")
        ...
        try:
            result = subprocess.run(
                ["cmake", "."], cwd=temp_dir, capture_output=True, text=True, check=True
            )
...
def main():
    ...
    parser.add_argument("--skip-test", action="store_true", help="Skip CMake test")
    ...
    if total_issues == 0:
        ...
        if args.fix:
            logger.info(
                "Some issues were automatically fixed. Please run the verification again."
            )
        else:
