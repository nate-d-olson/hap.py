...
- `tests/data/example/`: Example usage data and test data sets
...
- **Environment management:** micromamba (recommended) or conda/mamba.
  - Core dependencies are in `environment.yml`.
  - Development dependencies (for testing, linting, etc.) are in `environment-dev.yml`.
...
This section summarizes recent changes and fixes, providing a quick overview of ongoing development. For a comprehensive historical record of all releases, including ticket numbers and detailed descriptions, please refer to `RELEASES.md`.
...
To bring `hap.py` to MVP (Minimum Viable Product) status, the focus will be on establishing a robust CI/CD pipeline, enhancing test coverage and reliability, providing reproducible deployment options through containerization, and improving user documentation.

### MVP Goal

Establish a stable, well-tested, and easily deployable `hap.py` package with comprehensive core functionality and clear user documentation.
...
### Phase 2: Deployment & Reproducibility

**Objective**: Provide easy-to-use and reproducible deployment options.

*   **2.1 Finalize Conda Environment**:
    *   Review `environment.yml` and `environment-dev.yml` for completeness and accuracy.
    *   Ensure all necessary dependencies (including bioinformatics tools like `bcftools`, `samtools`, `rtg-tools`) are correctly specified and installable via Conda.
    *   Provide clear instructions for creating and activating the Conda environment.
*   **2.2 Develop Official Docker Image**:
    *   Create a `Dockerfile` that builds a production-ready `hap.py` image.
    *   Ensure the Docker image includes all runtime dependencies and `hap.py` itself.
    *   Provide instructions for building and running `hap.py` using Docker.
    *   Consider a multi-stage build for smaller image size.

### Phase 3: User Experience & Documentation

**Objective**: Enhance user documentation to facilitate adoption and usage.

*   **3.1 Expand User Guide (`docs/happy.md`)**:
    *   Add more detailed examples for common use cases (germline, somatic, stratification).
    *   Clearly explain input/output file formats and their interpretation.
    *   Provide guidance on performance tuning (memory, threads, engine selection).
*   **3.2 Update `mkdocs.yml`**:
    *   Ensure all moved documentation files (`docs/*.md`, `docs/api/`, `docs/migration/`, `docs/testing/`) are correctly integrated into the MkDocs navigation.
    *   Review and update the MkDocs theme and plugins for a modern and user-friendly experience.
*   **3.3 Create Example Usage Guides**:
    *   Develop small, self-contained tutorials in the `tests/data/example/` directory (or a new `examples/` directory if preferred for user-facing examples) demonstrating key `hap.py` functionalities.
    *   Include sample data and command-line snippets.
*   **3.4 Review and Refine `README.md`**:
    *   Ensure the "Quick Start" section is truly quick and easy to follow.
    *   Update links to point to the new `docs/` structure.
    *   Ensure all installation methods are clearly described.

### Success Metrics for MVP

*   All CI checks pass consistently.
*   Unit test coverage for core modules is at least 80%.
*   Conda environment and Docker image build successfully and run `hap.py` correctly.
*   User documentation is comprehensive and easy to navigate (verified by a quick review).
*   Basic usage examples are clear and executable.
