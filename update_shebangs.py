...
def update_shebang(file_path):
    ...
    if content.startswith('#!/usr/bin/env python') and not content.startswith('#!/usr/bin/env python3'):
        content = content.replace('#!/usr/bin/env python', '#!/usr/bin/env python3', 1)
        # Add encoding header if missing
        if 'coding: utf-8' not in content[:100] and '# -*- coding:' not in content[:100]:
            lines = content.split('\n')
